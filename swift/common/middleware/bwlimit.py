...
def interpret_conf_limits(conf, name_prefix, info=None):
    ...
    while conf_limits:
        ...
        bwlimits.append((cur_size, cur_rate, line_func))
    if info is None:
        return bwlimits
    else:
        return bwlimits, conf_limits_info


def get_maxrate(bwlimits, size):
    """
    ...
    if size:
        size = int(size)
        for ratesize, rate, func in bwlimits:
            if size < ratesize:
                break
            last_func = func
        if last_func:
...
class BwLimitMiddleware(object):
    """
    Bw limiting middleware

    Bw limits requests on both an Account and Container level.  Limits are
    configurable.
    """
    ...
    def __init__(self, app, conf, logger=None):
        ...
        self.account_ratelimit = float(conf.get('account_bwlimit', 0))
        ...
        self.bw_buffer_seconds = int(conf.get('bw_buffer_seconds', 5))
        self.bwlimit_whitelist = \
            [acc.strip() for acc in
                conf.get('account_whitelist', '').split(',') if acc.strip()]
        self.bwlimit_blacklist = \
            [acc.strip() for acc in
                conf.get('account_blacklist', '').split(',') if acc.strip()]
        self.container_bwlimits = interpret_conf_limits(
            conf, 'container_bwlimit_')
        self.container_listing_bwlimits = interpret_conf_limits(
            conf, 'container_listing_bwlimit_')
    ...
    def handle_bwlimit(self, req, account_name, container_name, obj_name):
        '''
        ...
        try:
            ...
            account_global_ratelimit = \
                account_info.get('sysmeta', {}).get('global-write-bwlimit')
        except ValueError:
        ...
        for key, max_rate in self.get_bwlimitable_key_tuples(
                req, account_name, container_name=container_name,
                obj_name=obj_name, global_bwlimit=account_global_bwlimit):
            try:
                need_to_sleep = self._get_sleep_time(key, max_bw)
                if self.log_sleep_time_seconds and \
                        need_to_sleep > self.log_sleep_time_seconds:
                    self.logger.warning(
                        _("Bwlimit sleep log: %(sleep)s for "
    ...
    def __call__(self, env, start_response):
        ...
        try:
         ...
         #   self.logger.error ("BWLimit : %(op)s %(version)s - %(account)s - %(container)s - %(obj)s ",{'op':req.method,'version':version, 'account':account, 'container':container, 'obj':obj})
        except ValueError:
            return self.app(env, start_response)

        #bwlimit_resp = self.handle_ratelimit(req, account, container, obj)
        bwlimit_resp = None
        if bwlimit_resp is None:
            result = self.app(env,start_response)

            size = (len(chunk) for chunk in result)
           # self.logger.error ("BWLimit (RET): %(op)s %(version)s - %(account)s - %(container)s - %(obj)s ",{'op':req.method,'version':version, 'account':account, 'container':container, 'obj':size})

            return result
        else:
            return bwlimit_resp(env, start_response)
...
def filter_factory(global_conf, **local_conf):
    ...
    account_bwlimit = float(conf.get('account_bwlimit', 0))
    max_sleep_time_seconds = \
        float(conf.get('max_sleep_time_seconds', 60))
    container_bwlimits, cont_limit_info = interpret_conf_limits(
        conf, 'container_bwlimit_', info=1)
    container_listing_ratelimits, cont_list_limit_info = \
        interpret_conf_limits(conf, 'container_listing_bwlimit_', info=1)
    # not all limits are exposed (intentionally)
    register_swift_info('bwlimit',
                        account_bwlimit=account_bwlimit,
                        max_sleep_time_seconds=max_sleep_time_seconds,
                        container_bwlimits=cont_limit_info,
                        container_listing_bwlimits=cont_list_limit_info)

    def bwlimit_filter(app):
        return BwLimitMiddleware(app, conf)

    return bwlimit_filter
